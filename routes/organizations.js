    ...
    .get(async (req, res) => {
        ...
        try {
            ...
            if (Org.members.some((mem) => mem.userName === req.session.user.userName)) {
                ...
                return res
                    .status(403)
                    .render("error.handlebars", { error_class: "input_error", message: "You must be a member of this organization in order to leave it!", error_route: req.session.currentPage });
            }
        } catch (e) {
            return res.status(400).render("error.handlebars", { error_class: "input_error", message: e, error_route: req.session.currentPage });
        }
