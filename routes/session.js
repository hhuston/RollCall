...
router
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      if (
        !Org.members.some((mem) => mem.userName === req.session.user.userName)
      ) {
        return res.status(403).render("error.handlebars", {
          error_class: "input_error",
      ...
      res.status(400).render("error.handlebars", {
  ...
  .post(async (req, res) => {
    ...
    try {
      let orgName = validation.checkOrgName(req.params.orgName);
      let proposal = validation.checkString(
        req.body.firstProposal,
        "Original Proposal"
      );
      let seshName = validation.checkString(req.body.seshName, "Session Name");
      let resp = await sessionData.createSession(
        proposal,
...
router
    ...
    .get(async (req, res) => {
      ...
      if (
        ...
        return res.status(400).render("error.handlebars", {
          error_class: `bad_param`,
          message: `You are not a member of ${Sesh.orgName}`,
          error_route: req.session.currentPage,
        });
      }
      if (
        Sesh.members.some((mem) => mem.userName === req.session.user.userName)
      ) {
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      let resp = sessionData.joinSession(
        sessionId,
        role,
        req.session.user.userName
      );
...
router
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      let queuedActions = actions.filter(
        (action) => action.status === "queued"
      );
      let oncallActions = actions.filter(
        (action) => action.status === "oncall"
      );
      let loggedActions = actions.filter(
        (action) => action.status === "logged"
      );
      ...
      if (Sesh.open) {
        return res.render("session.handlebars", {
          ...
          oncallActions: oncallActions,
          loggedActions: loggedActions,
        });
      } else {
        return res.render("listofactions.handlebars", {
          actions: actions,
        });
      }
    } catch (e) {
